...
@Repository
public class DiscoverSectionRepositoryImpl implements DiscoverSectionRepository {
	...
	public Object getById(String beanClassName, int id) {
		...
		Object object = null;
		try {
			Query<?> query = session.createQuery("FROM " + beanClassName + " WHERE deletionFlag=?0 AND discoverSectionId=?1");
			query.setParameter(0, 0);
